...
async function main() {
  ...
  const user = await prisma.user.upsert({
    where: { email: "verify-neon@test.com" },
    update: {},
    create: { email: "verify-neon@test.com" },
  });
  ...
  const created = await prisma.todo.create({
    data: { title: "Test verify create", userId: user.id },
  });
  ...
  await prisma.todo.create({ data: { title: "Test verify second", userId: user.id } });

  // 3. READ: List all todos for this user
  console.log("2. READ: listing all todos...");
  const allTodos = await prisma.todo.findMany({ where: { userId: user.id }, orderBy: { id: "asc" } });
  ...
  const remaining = await prisma.todo.findMany({ where: { userId: user.id } });
  for (const t of remaining) {
    await prisma.todo.delete({ where: { id: t.id, userId: user.id } });
  }

  const final = await prisma.todo.findMany({ where: { userId: user.id } });
